...
def findPairValues(array)
	@length = array.length
	@targetSum = 6

	for i in 0...@length
		a1 = array[i]
		a2 = @targetSum - array[i]
		puts (a1, a2 )
	end
end

def selectionSort(array)
	array.length.times do |startinElement|
		minimum = startinElement
		startinElement.upto(array.length - 1) do |i|
			minimum = i if array[i] < array[min]
		end
		array[startinElement], array[minimum] = array[minimum], array[startinElement]
	end
	array
end
